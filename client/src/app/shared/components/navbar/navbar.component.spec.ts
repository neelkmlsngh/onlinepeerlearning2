...
let de, deditor, dforums, dprofile, dlogout, dsnippet:      DebugElement;
let el, eeditor, eforums, eprofile, elogout, esnippet:      HTMLElement;
let spy, spy1;
...
beforeEach(() => {
   ...
   fixture.detectChanges();
   expect(eeditor.textContent).toContain(config.navbar.ONLINECODEEDITOR);
   });
   ...
   spy = spyOn(authenticationService,'logoutEditor').and.returnValue(Observable.of(authdata));
   component.logout();
   expect(tests.response.n).not.toEqual(1);
   expect(tests.response.nModified).not.toEqual(1);
   expect(tests.response.ok).not.toEqual(1);
   });
})
